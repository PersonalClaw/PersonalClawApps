"""Unit tests for the together model provider app.
...
def test_type_and_catalog_registered():
    ...
    assert reg.capability_of("together").type == "together"
    assert reg.catalog_of("together") is not None


def test_spec_defaults():
    assert prov.SPEC.type == "together"
    assert prov.SPEC.default_base_url == "https://api.together.xyz/v1"
    assert prov.SPEC.api_key_env == "TOGETHER_API_KEY"
...
def test_registry_build(monkeypatch):
    reg = get_default_registry()
    if not any(e.name == "together-inst" for e in reg.list_entries()):
        reg.register_entry(ProviderEntry(
            name="together-inst", type="together", model="m",
            options={"api_key": "k"},
            declared_capabilities=frozenset({Capability.CHAT}),
        ))
    p = reg.build("together-inst")
    assert p._base_url == "https://api.together.xyz/v1"
