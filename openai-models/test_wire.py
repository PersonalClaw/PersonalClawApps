"""OpenAI: what core asks of a call is in the request the model receives.
...
The factory is this app's own. So this drives the app's real ``openai`` client against a fake
...
def _entry(url: str) -> ProviderEntry:
    ...
    return ProviderEntry(name="wire", type="openai", model="", options=form_options(APP_DIR, url))
