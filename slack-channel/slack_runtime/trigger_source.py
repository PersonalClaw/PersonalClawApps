...
namespaces them ``app:slack-channel:<event>``, fences the text at origin with full provenance,
...
:meth:`SlackTriggerSource.start`. There is no ``emit_event``, no ``event_bus``, no
...
# sibling ``slack_runtime.*`` import below must keep resolving for the life of the process
...
from slack_runtime.inbound_tap import subscribe, unsubscribe

logger = logging.getLogger(__name__)

#: The registered source name, and therefore the middle segment of every namespaced event
#: (``app:slack-channel:<event>``). The APP name rather than the vendor word: the SDK contract
...
class SlackTriggerSource(TriggerSourceProvider):
...
def create_provider(config: dict[str, Any] | None = None) -> SlackTriggerSource:
    """Manifest factory — mirrors ``transport.create_provider``'s contract."""
    return SlackTriggerSource(config)
