...
* **#953** — ``allowed_users`` was parsed into :class:`SlackSettings` and then ignored: the
  runtime seeded its allowlist from ``PERSONALCLAW_OWNER_ID`` alone, and the authorization
...
* ``test_schema_and_dataclass_are_symmetric`` is exact in both directions, and is the only
  rail that can see #952 at all — a token is not a ``SlackSettings`` field, so
...
from slack_runtime.settings import CREDENTIAL_SETTING_KEYS, SlackSettings, load_tokens, reload_settings
from slack_runtime.transport import SlackTransport

_APP_DIR = Path(__file__).resolve().parents[1]
_SETTINGS_FIELDS = tuple(f.name for f in fields(SlackSettings))
...
def test_schema_and_dataclass_are_symmetric():
    ...
    assert declared - modelled == set(), (
        ...
        "nothing behind it. Add them to SlackSettings (behaviour) or to "
...
def _settings_accessors() -> dict[str, str]:
    """``{method: source}`` for ``SlackSettings``' own public methods, minus the parser.
    ...
    """
    out: dict[str, str] = {}
    for name, member in vars(SlackSettings).items():
...
def test_every_settings_field_is_read_outside_settings_py(field_name):
    """A ``SlackSettings`` field must be consumed somewhere other than where it is parsed.
...
def test_a_credential_setting_reaches_the_runtime_from_the_store_alone(key, store_only_home):
    ...
    runtime = SlackRuntime(_Services(), config=store)
    assert runtime._slack_enabled, (
