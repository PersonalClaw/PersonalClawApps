"""SlackTriggerSource — the bundle's third provider (CE-10).
...
def _orch(channels: dict[str, ChannelConfig] | None = None) -> MagicMock:
    ...
    orch = MagicMock()
    settings = SlackSettings(channels=channels or {}, dm_activation=ACTIVATION_ALWAYS)
    ...
    orch.slack.get_user_info = AsyncMock(return_value={"real_name": "Ada"})
    orch.slack.post_ephemeral = AsyncMock()
    orch.slack.add_reaction = AsyncMock()
...
def test_the_namespace_segment_is_the_APP_name_not_the_vendor_registry_name():
    ...
    assert SlackTransport({}).name == "slack"
    assert SlackInboxSource({}).source_name == "slack"
    assert SlackTriggerSource({}).name == "slack-channel"
...
def test_the_routers_normalizer_carries_NO_display_name(gate):
    ...
    """
    from slack_runtime.events import publish_inbound_for_trigger_source
