"""Discord's ``trigger_source`` provider — real inbound traffic as automation events.
...
The app observes something core knows nothing about (a Discord guild or DM) and hands core
typed :class:`~personalclaw.sdk.trigger_source.SourceEvent`\\ s. Core does everything else:
it namespaces them ``app:discord-channel:<event>``, fences the text at origin with full
...
:meth:`DiscordTriggerSource.start`. There is no ``emit_event``, no ``event_bus``, no
...
1. **Only trust-gated traffic is observed.** The transport publishes to ``inbound_tap`` on
   the path where core's guarded door already returned ``allowed`` — so anyone who merely
   shares a guild with the bot gets no session AND arms no automation. A test drives the
   denied path.
2. **The event NAME comes from a closed vocabulary in this file** (:data:`EVENTS`), chosen
   by a STRUCTURAL fact the transport already computed (``is_dm``, from the presence of
   ``guild_id``) and never from anything in the message. A source that let inbound data
   pick its own event name would let a poster choose which of the owner's triggers to
   match, which is choosing the action — the same hazard the ``companion`` app avoids by
   synthesising its trigger rows in code so no field in its file can name an action. There
   is no field in a Discord message that names an event here either.
3. **Prose goes in ``text`` and nowhere else.** ``text`` is the one field core fences at
   ...
   identifiers and a boolean — no ``global_name``, no ``username``, no body. A Discord
   display name is chosen by whoever owns the account, and prose that skipped the fence is
   prose that arrives as instructions.
...
# sibling ``discord_runtime.*`` import below must keep resolving for the life of the process
...
from discord_runtime.inbound_tap import subscribe, unsubscribe

logger = logging.getLogger(__name__)

#: The registered source name, and therefore the middle segment of every namespaced event
#: (``app:discord-channel:<event>``). The APP name rather than the vendor word: the SDK
...
class DiscordTriggerSource(TriggerSourceProvider):
...
def create_provider(config: dict[str, Any] | None = None) -> DiscordTriggerSource:
    """Manifest factory — mirrors ``transport.create_provider``'s contract."""
    return DiscordTriggerSource(config)
