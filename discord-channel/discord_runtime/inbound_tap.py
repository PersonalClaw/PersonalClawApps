...
to, because the alternative is a **second** Discord Gateway connection: Discord counts
identifies per bot token and would rate-limit us, the two sockets would resume from
different sequence numbers, and core would be told about every message twice. The same
reasoning made ``inbox_source`` in the slack bundle an adapter over the existing client
...
``discord_runtime.inbound_tap`` is one object in ``sys.modules`` for the life of the
process, exactly like ``discord_runtime.settings``' cache.
...
would let anyone who shares a guild with the bot arm the owner's automations by posting,
which is a strictly worse failure than the one the trust gate exists to prevent: a denied
stranger gets no session AND must get no trigger fire. The transport calls :func:`publish`
on the allowed path only, and a test drives the denied path to prove nothing is published.
