...
``DISCORD_BOT_TOKEN``. No uninstall can attribute a plain name to an app, so removing the app
...
def test_setup_keeps_the_token_under_a_key_this_app_owns(home):
    ...
    assert CRED_DISCORD_BOT_TOKEN not in credentials.credential_names()
