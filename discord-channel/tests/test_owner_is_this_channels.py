...
Discord writes and reads its own. An install from an earlier release, with the owner only under the
shared key, moves it to Discord's own key the first time the channel starts. Driven through the real
setup step, doctor probe and transport, against the real credential store in this test's home.
"""
...
import cli_doctor
import cli_setup
import discord_runtime.settings as settings_mod
...
@pytest.mark.asyncio
async def test_the_transport_files_its_delivery_under_discords_own_owner(monkeypatch, caplog):
    """Core files the delivery under "discord" and reads Discord's own owner by it, and the
    handle DMs that owner, not the Telegram id under the shared key. Its own key is left alone."""
...
def test_setup_keeps_an_earlier_releases_owner_under_the_own_key(monkeypatch):
    ...
    stores it under Discord's own key."""
    monkeypatch.setenv(CRED_DISCORD_BOT_TOKEN, TOKEN)
...
async def test_an_install_from_an_earlier_release_adopts_its_owner_once(monkeypatch, caplog):
    """The first start stores the owner the shared key holds under Discord's own key, so it
    ...
    assert _OWN in logged[0] and "112233445566778899" not in logged[0]
    assert credentials.get_credential(_SHARED) == "112233445566778899", "the shared key was changed"
...
async def test_an_install_with_no_owner_starts_as_before(monkeypatch, caplog):
    ...
    assert await _what_the_transport_registers(monkeypatch) == ("discord", "")
