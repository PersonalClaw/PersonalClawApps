...
:meth:`TelegramTriggerSource.start`. There is no ``emit_event``, no ``event_bus``, no
...
1. **Only trust-gated traffic is observed.** The transport publishes to
   ...
   ``allowed`` — so a stranger who messages the bot gets no session AND arms no
   automation. The tap's module docstring carries the reasoning; a test drives the
   denied path.
2. **The event NAME comes from a closed vocabulary in this file** (:data:`EVENTS`),
   chosen by a STRUCTURAL fact the transport already computed (``is_dm``) and never
   from anything in the message. A source that let inbound data pick its own event
   name would let a sender choose which of the owner's triggers to match, which is
   choosing the action — the same hazard the ``companion`` app avoids by synthesising
   its trigger rows in code so no field in its file can name an action. There is no
   field in a Telegram message that names an event here either.
3. **Prose goes in ``text`` and nowhere else.** ``text`` is the one field core fences
   ...
   four identifiers and a boolean — no display name, no body, no subject. A remote
   sender's chosen display name is prose, and prose that skipped the fence is prose
   that arrives as instructions.
...
# sibling ``telegram_runtime.*`` import below must keep resolving for the life of the
...
from telegram_runtime.inbound_tap import subscribe, unsubscribe

logger = logging.getLogger(__name__)

#: The registered source name, and therefore the middle segment of every namespaced event
#: (``app:telegram-channel:<event>``). The APP name rather than the vendor word: the SDK
...
#: updates.
APP_NAME = "telegram-channel"

#: A message in a private chat.
EVENT_DIRECT_MESSAGE = "direct_message"
#: A message in a group or supergroup the bot is in.
EVENT_GROUP_MESSAGE = "group_message"

#: The CLOSED event vocabulary — what a user can bind a trigger to, and the only names
#: this source will ever emit. Declared (not discovered) so the trigger-create surface
#: offers names that can actually fire; frozen in code so no inbound field can add one.
EVENTS: tuple[str, ...] = (EVENT_DIRECT_MESSAGE, EVENT_GROUP_MESSAGE)
...
META_KEYS: tuple[str, ...] = ("channel_id", "sender", "chat_type", "is_dm")


class TelegramTriggerSource(TriggerSourceProvider):
    """Telegram's inbound messages, as trigger events.

    A PUSH source: it subscribes to the bundle's inbound tap and calls the ``emit``
    callable core gave it. Core never polls it — the poll cadence, the rate limits and the
    ``getUpdates`` offset are the transport's, and they stay there.
    """
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        # setting. A settings block here would be a second place to turn Telegram on.
    ...
    async def start(self, emit: Any) -> None:
        ...
        No watch loop of its own: the transport already owns the long-poll, and a second
        one would be a second Telegram connection. ``start`` runs on the app-enable path,
        so it must not block.
        """
        self._emit = emit
        subscribe(self._on_inbound)
        logger.info("telegram trigger source attached (%d event names)", len(EVENTS))
    ...
    def _on_inbound(self, message: Any, *, is_dm: bool) -> None:
        ...
        the conversation turn the message was really for. The tap logs and moves on; this
        method additionally refuses to emit when it has no ``emit`` callable rather than
        raising, because "the source is not started" is a normal state (a message can
        arrive between ``stop`` and the app's actual shutdown).
        """
        emit = self._emit
        if emit is None:
            return
        ...
        if not text.strip():
            # Nothing to match on. A stickers-and-media event with an empty payload would
            # fire every content-matching trigger's catch-all and tell the user nothing.
            return
        metadata = getattr(message, "metadata", None) or {}
        # STRUCTURAL, not textual: `is_dm` is the transport's own computation from the
        # chat type, so no message field selects the event name. See rule 2 in the module
        # docstring.
        event = EVENT_DIRECT_MESSAGE if is_dm else EVENT_GROUP_MESSAGE
        emit(
            SourceEvent(
                event=event,
                # The vendor's own message id, so two fires of the same event are
...
def create_provider(config: dict[str, Any] | None = None) -> TelegramTriggerSource:
    """Manifest factory — mirrors ``transport.create_provider``'s contract."""
    return TelegramTriggerSource(config)
