...
to, because the alternative is a **second** Telegram connection: core would then be told
about every message twice, Telegram would rate-limit us for polling ``getUpdates`` from
two offsets, and the two views would disagree about what arrived. The same reasoning made
``inbox_source`` in the slack bundle an adapter over the existing client rather than a
second client.

So the transport publishes here and the trigger source subscribes here. A module in this
package is the only thing the two instances demonstrably share: the app loader pins the
app dir on ``sys.path`` (see the header of ``transport.py``), and after that
``telegram_runtime.inbound_tap`` is one object in ``sys.modules`` for the life of the
process, exactly like ``telegram_runtime.settings``' cache.
...
would let anyone who can reach the bot arm the owner's automations by sending it a
message, which is a strictly worse failure than the one the trust gate exists to prevent:
a denied stranger gets no session AND must get no trigger fire. The transport calls
:func:`publish` on the allowed path only, and a test drives the denied path to prove
nothing is published.

**No event glue lives here.** This module knows nothing about events, the bus, or
triggers — it moves a :class:`~personalclaw.sdk.channel.ChannelMessage` between two
objects in one process. Turning one into an event is the trigger source's job, and it does
it by calling the ``emit`` callable core handed it.
"""
...
#: grew vendor payload fields would pull Telegram's update schema into every subscriber.
...
def subscribe(observer: Observer) -> None:
    ...
    enabled, disabled and re-enabled must not receive each message twice — a doubled
    event doubles the fires the user sees, and the debounce would hide it just often
    enough to make it hard to find.
    """
...
def publish(message: Any, *, is_dm: bool) -> int:
    ...
    for observer in list(_observers):
        try:
            ...
            logger.debug("telegram: inbound observer failed", exc_info=True)
    return delivered
