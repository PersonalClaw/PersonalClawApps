...
``delete_model``). Sending a Telegram message is exactly that class of write — it is
outward-facing, instantly visible to a human, and un-sendable — so this transport
...
class SendRefused:
    ...
    #: The channel that refused (``"telegram"``) — set so a log line or an aggregated
    #: refusal is attributable when several transports are live.
    channel: str
    #: The chat/channel id the message WOULD have gone to.
    target: str
