...
async def test_unsigned_request_is_refused_but_health_is_exempt(client):
    # A raw (unsigned) request to a real route is refused fail-closed.
    assert (await client.raw.get("/meetings")).status == 401
    assert (await client.raw.post("/meetings", json={"title": "x"})).status == 401
    # /health is exempt (the gateway watchdog probes it directly).
    assert (await client.raw.get("/health")).status == 200
