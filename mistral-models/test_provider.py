"""Unit tests for the mistral model provider app.
...
def test_type_and_catalog_registered():
    ...
    assert reg.capability_of("mistral").type == "mistral"
    assert reg.catalog_of("mistral") is not None


def test_spec_defaults():
    assert prov.SPEC.type == "mistral"
    assert prov.SPEC.default_base_url == "https://api.mistral.ai/v1"
    assert prov.SPEC.api_key_env == "MISTRAL_API_KEY"
...
def test_registry_build(monkeypatch):
    reg = get_default_registry()
    if not any(e.name == "mistral-inst" for e in reg.list_entries()):
        reg.register_entry(ProviderEntry(
            name="mistral-inst", type="mistral", model="m",
            options={"api_key": "k"},
            declared_capabilities=frozenset({Capability.CHAT}),
        ))
    p = reg.build("mistral-inst")
    assert p._base_url == "https://api.mistral.ai/v1"
