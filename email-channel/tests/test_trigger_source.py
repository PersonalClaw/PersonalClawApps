"""EmailTriggerSource — the bundle's ``trigger_source`` provider (CE-10).

Every clause here is DRIVEN. The end-to-end test starts from a raw RFC822 message sitting in
a fake IMAP mailbox and ends at a real action provider receiving a fire, through: the real
...
from personalclaw.sdk.channel import ProviderSettings, allow_sender, save_credential
...
from email_runtime.trigger_source import (
    APP_NAME,
    EVENT_MAIL_RECEIVED,
    EVENTS,
    META_KEYS,
...
def test_the_event_carries_the_QUOTE_STRIPPED_text_not_the_raw_body(
    ...
    """A reply's event must carry its NEW prose, not the history below it.
...
def test_the_event_NAME_is_NEVER_TAKEN_FROM_THE_MAIL(registered_source):
    ...
    There is exactly one name and it is a constant in this file, so a mail whose subject,
...
def test_meta_carries_IDENTIFIERS_ONLY_never_prose_and_the_SUBJECT_IS_ABSENT(registered_source):
    ...
    attacker-chosen prose, and unfenced prose is prose that arrives as instructions. The
    consequence — no subject-matching trigger today — is recorded in the module docstring
    rather than traded away, and asserted here so a future "just add the subject" cannot land
    quietly.
    """
    seen: list = []
    registered_source._emit = seen.append
