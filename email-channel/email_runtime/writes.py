...
``delete_model``). Handing a message to an SMTP server is the
most irreversible write in this repo: once the server accepts it there is no
recall, no edit and no delete — so this transport honors the same switch.
...
class SendRefused:
    ...
    #: The channel that refused (``"email"``) — set so a log line or an aggregated
