...
``email_runtime.inbound_tap`` is one object in ``sys.modules`` for the life of the process,
exactly like ``email_runtime.settings``' cache.
...
would let anyone who knows the mailbox address arm the owner's automations by sending mail —
and a ``From`` address is trivially forged, which is exactly why the address allowlist is
core's decision and not this module's. A denied sender gets no session AND must get no
trigger fire. The transport calls :func:`publish` on the allowed path only, and a test
drives the denied path to prove nothing is published.
...
def publish(message: Any, *, text: str) -> int:
    ...
    and the next observer still runs. Returning the count rather than nothing keeps a caller
    able to log honestly instead of assuming delivery.
    """
    delivered = 0
    for observer in list(_observers):
        try:
            ...
            logger.debug("email: inbound observer failed", exc_info=True)
    return delivered
