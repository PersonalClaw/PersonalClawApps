"""Email's ``trigger_source`` provider — real inbound mail as automation events.
...
namespaces them ``app:email-channel:<event>``, fences the text at origin with full
provenance, and matches them against the user's ``kind: event`` triggers through the existing
``{source, pattern}`` spec.

**Nothing here reaches the bus.** The only outward call is the ``emit`` callable core hands
:meth:`EmailTriggerSource.start`. There is no ``emit_event``, no ``event_bus``, no
...
message: there is no room concept, so the transport passes ``is_dm=True`` unconditionally and
there is no second structural fact to name a second event with. Declaring
...
2. **The event NAME is a constant in this file** (:data:`EVENTS`), never read from the mail.
   A source that took its event name from a header would let a sender choose which of the
   owner's triggers to match, which is choosing the action — the same hazard the
   ``companion`` app avoids by synthesising its trigger rows in code so no field in its file
   can name an action. There is no header that names an event here either.
3. **Prose goes in ``text`` and nowhere else.** ``text`` is the one field core fences at
   ingestion, and it carries the quote-STRIPPED new prose the door itself received — not the
   raw body, whose quoted history is mostly our own previous words. ``meta`` is matched, not
   narrated, and is built literally below from two identifiers and a boolean.
   ...
   **The ``Subject`` is therefore ABSENT from ``meta``, deliberately.** A subject is
   ...
   subject-matching trigger is not expressible today; the limit is recorded here rather than
...
# sibling ``email_runtime.*`` import below must keep resolving for the life of the process
...
#: (``app:email-channel:<event>``). The APP name rather than the vendor word: the SDK
#: contract says the identifier is "matched to the app name", core derives the namespace from
#: the REGISTERED name, and a trigger bound to it must keep matching across app updates.
APP_NAME = "email-channel"
...
class EmailTriggerSource(TriggerSourceProvider):
    """Inbound mail, as trigger events.

    A PUSH source: it subscribes to the bundle's inbound tap and calls the ``emit`` callable
    core gave it. Core never polls it — the IMAP poll cadence, the UID cursor and the
    reconnect backoff are the transport's, and they stay there.
    """
    ...
    async def start(self, emit: Any) -> None:
        ...
        No poll loop of its own: the transport already owns the IMAP connection, and a second
        one would be a second cursor over the same mailbox. ``start`` runs on the app-enable
        path, so it must not block.
        """
        self._emit = emit
        subscribe(self._on_inbound)
        logger.info("email trigger source attached (%d event names)", len(EVENTS))
    ...
    def _on_inbound(self, message: Any, *, text: str) -> None:
        ...
        conversation turn the mail was really for. This method additionally refuses to emit
        when it has no ``emit`` callable rather than raising, because "the source is not
        started" is a normal state (mail can arrive between ``stop`` and the app's actual
        shutdown).
        """
        emit = self._emit
        if emit is None:
        ...
        emit(
            SourceEvent(
                ...
                text=body,
                meta={
                    # The correspondent's address — how core addresses a reply back — which
                    # is an identifier, not prose. The DISPLAY name is not here.
...
def create_provider(config: dict[str, Any] | None = None) -> EmailTriggerSource:
    """Manifest factory — mirrors ``transport.create_provider``'s contract."""
    return EmailTriggerSource(config)
