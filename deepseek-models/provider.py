"""DeepSeek model provider (standalone app).

Speaks the **OpenAI-compatible** inference protocol over DeepSeek's endpoint
(https://api.deepseek.com/v1). The wire client (``OpenAIProvider``) is a supported standard that lives
...
Bring your own API key (config ``api_key`` or the ``DEEPSEEK_API_KEY`` environment variable).
"""
