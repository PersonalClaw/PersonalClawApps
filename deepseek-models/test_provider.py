"""Unit tests for the deepseek model provider app.
...
def test_type_and_catalog_registered():
    ...
    assert reg.capability_of("deepseek").type == "deepseek"
    assert reg.catalog_of("deepseek") is not None


def test_spec_defaults():
    assert prov.SPEC.type == "deepseek"
    assert prov.SPEC.default_base_url == "https://api.deepseek.com/v1"
    assert prov.SPEC.api_key_env == "DEEPSEEK_API_KEY"
...
def test_registry_build(monkeypatch):
    reg = get_default_registry()
    if not any(e.name == "deepseek-inst" for e in reg.list_entries()):
        reg.register_entry(ProviderEntry(
            name="deepseek-inst", type="deepseek", model="m",
            options={"api_key": "k"},
            declared_capabilities=frozenset({Capability.CHAT}),
        ))
    p = reg.build("deepseek-inst")
    assert p._base_url == "https://api.deepseek.com/v1"
